...
/*   Updated: 2023/10/10 16:54:07 by lorobert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::map<int, std::string> Server::getRequests() const
{
	return (_requests);
}
...
Server::Server(Server const &other)
    : _config(other._config), _name(other._name), _host(other._host),
      _port(other._port), _socket(other._socket), _addr(other._addr) {}
...
bool Server::setup() {
  ...
  int option_value = 1;
  if (setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR, &option_value,
                 sizeof(int)) < 0) {
